...
extern "C"{
void trackKernel(TrackData* output, float4* inVertex,
	...
	for (pixely = 0; pixely < inSize_y; pixely++) {
		...
		for (pixelx = 0; pixelx < inSize_x; pixelx++) {
			...
			// idx *= 7;
			output[idx].error = dot(referenceNormal, diff);

			float3 cross_res = cross(projectedVertex, referenceNormal);

			output[idx].J[0] = referenceNormal.x;
			output[idx].J[1] = referenceNormal.y;
			output[idx].J[2] = referenceNormal.z;
			output[idx].J[3] = cross_res.x;
			output[idx].J[4] = cross_res.y;
			output[idx].J[5] = cross_res.z;
		}
	}
}
}
