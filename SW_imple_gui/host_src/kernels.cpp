...
#include <mach/mach.h>
	...
	#define TOCK(str,size)  {if (print_kernel_timing) {clock_gettime(CLOCK_MONOTONIC, &tock_clockData);\
		std::cout  << (( tock_clockData.tv_sec - tick_clockData.tv_sec) + ((tock_clockData.tv_nsec-tick_clockData.tv_nsec)/1000000000.f)) << "\t"; }}
...
void Kfusion::languageSpecificConstructor() {
	...
	std::cout << "frame\tbilateral\thalfsample\tdepth2vertex\tvertex2normal\ttrackiANDreduce\tintegrate\n";
...
bool Kfusion::tracking(float4 k, float icp_threshold, uint tracking_rate,
	...
	TOCK("depth2vertexKernel", 0);
	
	TICK();
	localimagesize = computationSize;
	for (unsigned int i = 0; i < iterations.size(); ++i) {
		vertex2normalKernel(inputNormal[i], inputVertex[i], localimagesize);
		localimagesize = make_uint2(localimagesize.x / 2, localimagesize.y / 2);
	}
	TOCK("vertex2normalKernel", 0);
