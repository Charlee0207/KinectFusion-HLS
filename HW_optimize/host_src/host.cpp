...
int main(int argc, char ** argv) {
	...
	// Allocate inputDepth buffer using Opencl buffer
	...
	uint frame = 0;
	Kfusion kfusion(/*inputSize, */computationSize, config.volume_resolution,
			config.volume_size, init_pose, config.pyramid,binaryFile);
	// ushort* inputDepth = kfusion.getInputDepthBuffer();
