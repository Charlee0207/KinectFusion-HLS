/*
 Created by Lee, Cheng-Hao - 2023 ITRI EOSL summer intern program.
...
void bilateralFilterKernel(float* out, float* in, uint size_x, uint size_y,
	...
	for (int y=0; y<HEIGHT; y++) {
		...
		#pragma HLS PIPELINE II=1
			...
			WINDOW_FOR: for(int j=0; j<WINDOW_SIZE; j++){
				#pragma HLS unroll
				mod[0][j] = sq(windowBuffer[(y<RADIUS)?2:0			][(colBoundaryCondition[j])?2:j] - windowBuffer[2][2]);
				mod[1][j] = sq(windowBuffer[(y<RADIUS-1)?2:1		][(colBoundaryCondition[j])?2:j] - windowBuffer[2][2]);
				mod[2][j] = sq(windowBuffer[2						][(colBoundaryCondition[j])?2:j] - windowBuffer[2][2]);
				mod[3][j] = sq(windowBuffer[(y>HEIGHT-RADIUS)?2:3	][(colBoundaryCondition[j])?2:j] - windowBuffer[2][2]);
				mod[4][j] = sq(windowBuffer[(y>HEIGHT-RADIUS-1)?2:4	][(colBoundaryCondition[j])?2:j] - windowBuffer[2][2]);
