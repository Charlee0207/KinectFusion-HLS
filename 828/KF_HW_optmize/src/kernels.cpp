...
cl_context context;
cl_command_queue Q, Q_bilateralFilterKernel, Q_mm2meter2bilateralFilterKernel, Q_halfSampleDepth2vertexKernel, Q_vertex2normalKernel, Q_trackKernel, Q_integrateKernel;
cl_program program;
cl_kernel K_bilateralFilterKernel, K_mm2meter2bilateralFilterKernel, K_halfSampleDepth2vertexKernel, K_vertex2normalKernel, K_trackKernel, K_trackANDreduceKernel, K_integrateKernel;
/////////////////////////////////////////////////////////////////////////
...
// float* trackingResult_float;
// int* trackingResult_data;
float4 ** inputVertex;
float4 ** inputNormal;
float4* vertex;
float4* normal;
...
// cl_mem trackingResult_BUF;
cl_mem reductionoutput_BUF;
// cl_mem trackingResult_float_BUF;
// cl_mem trackingResult_data_BUF;
...
ushort* Kfusion::getInputDepthBuffer(){
	return inputDepth;
}
void Kfusion::languageSpecificConstructor() {
	...
	OCL_CHECK(err, Q_mm2meter2bilateralFilterKernel = clCreateCommandQueue(context, device_id, CL_QUEUE_PROFILING_ENABLE, &err));
	...
	OCL_CHECK(err, Q_trackKernel = clCreateCommandQueue(context, device_id, CL_QUEUE_PROFILING_ENABLE, &err));
	...
	// OCL_CHECK(err, K_mm2meter2bilateralFilterKernel = clCreateKernel(program, "mm2meter2bilateralFilterKernel", &err));
	...
	// OCL_CHECK(err, K_trackKernel = clCreateKernel(program, "trackKernel", &err));
	...
	// OCL_CHECK(err, trackingResult_float_BUF = clCreateBuffer(context, CL_MEM_READ_WRITE,  7*sizeof(float)*compSize, NULL, &err));
	// OCL_CHECK(err, trackingResult_data_BUF = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int)*compSize, NULL, &err));
	// OCL_CHECK(err, trackingResult_BUF = clCreateBuffer(context, CL_MEM_READ_WRITE,  sizeof(TrackData)*compSize, NULL, &err));
	...
	// OCL_CHECK(err, trackingResult_float = (float *)clEnqueueMapBuffer(Q,trackingResult_float_BUF,true,CL_MAP_READ | CL_MAP_WRITE,0,7*sizeof(float)*compSize,0,nullptr,nullptr,&err));
	// OCL_CHECK(err, trackingResult_data = (int *)clEnqueueMapBuffer(Q,trackingResult_data_BUF,true,CL_MAP_READ | CL_MAP_WRITE,0,sizeof(int)*compSize,0,nullptr,nullptr,&err));
	// OCL_CHECK(err, trackingResult = (TrackData *)clEnqueueMapBuffer(Q,trackingResult_BUF,true,CL_MAP_READ | CL_MAP_WRITE,0,sizeof(TrackData)*compSize,0,nullptr,nullptr,&err));
...
Kfusion::~Kfusion() {
	...
	// OCL_CHECK(err, err = clEnqueueUnmapMemObject(Q,trackingResult_float_BUF,trackingResult_float,0,NULL,NULL));
	// OCL_CHECK(err, err = clEnqueueUnmapMemObject(Q,trackingResult_data_BUF,trackingResult_data,0,NULL,NULL));
	// OCL_CHECK(err, err = clEnqueueUnmapMemObject(Q,trackingResult_BUF,trackingResult,0,NULL,NULL));
	...
	// clReleaseMemObject(trackingResult_float_BUF);
	// clReleaseMemObject(trackingResult_data_BUF);
	// clReleaseMemObject(trackingResult_BUF);
    ...
    clReleaseCommandQueue(Q_trackKernel);
...
bool Kfusion::preprocessing(const ushort * inputDepth, const uint2 inputSize) {
	...
	// int argcount = 0;
	// cl_mem pt_mm2meter2bf_in[2] = {inputDepth_BUF, gaussian_BUF};
	// cl_mem pt_mm2meter2bf_out[1] = {scaledDepth0_BUF};
	// OCL_CHECK(err, err = clSetKernelArg(K_mm2meter2bilateralFilterKernel,argcount++, sizeof(cl_mem), &scaledDepth0_BUF));
	// OCL_CHECK(err, err = clSetKernelArg(K_mm2meter2bilateralFilterKernel,argcount++, sizeof(cl_mem), &inputDepth_BUF));
	// OCL_CHECK(err, err = clSetKernelArg(K_mm2meter2bilateralFilterKernel,argcount++, sizeof(uint), &inputSize.x));
	// OCL_CHECK(err, err = clSetKernelArg(K_mm2meter2bilateralFilterKernel,argcount++, sizeof(uint), &inputSize.y));
	// OCL_CHECK(err, err = clSetKernelArg(K_mm2meter2bilateralFilterKernel,argcount++, sizeof(uint), &computationSize.x));
	// OCL_CHECK(err, err = clSetKernelArg(K_mm2meter2bilateralFilterKernel,argcount++, sizeof(uint), &computationSize.y));
	// OCL_CHECK(err, err = clSetKernelArg(K_mm2meter2bilateralFilterKernel,argcount++, sizeof(cl_mem), &gaussian_BUF));
	// OCL_CHECK(err, err = clSetKernelArg(K_mm2meter2bilateralFilterKernel,argcount++, sizeof(float), &e_delta));
	// OCL_CHECK(err, err = clSetKernelArg(K_mm2meter2bilateralFilterKernel,argcount++, sizeof(int), &radius));
	
	
	// OCL_CHECK(err, err = clEnqueueMigrateMemObjects(Q_mm2meter2bilateralFilterKernel,(cl_uint)2, pt_mm2meter2bf_in, 0 ,0,NULL, NULL));
	// OCL_CHECK(err, err = clEnqueueTask(Q_mm2meter2bilateralFilterKernel, K_mm2meter2bilateralFilterKernel, 0, NULL, NULL));
	// OCL_CHECK(err, err = clEnqueueMigrateMemObjects(Q_mm2meter2bilateralFilterKernel,(cl_uint)1, &scaledDepth0_BUF, CL_MIGRATE_MEM_OBJECT_HOST,0,NULL, NULL));

	// clFinish(Q_mm2meter2bilateralFilterKernel);


		// static int counter=0;
		// std::string path = "/home/jason/KinectFusion_U50/KinectFusion_HW_impl/output/HW_impl/"; 
		// std::string name = path + "scaledDepth_" + std::to_string(counter); 
		// std::ofstream fResult(name, std::ios::out);
		
		// for(int y=0; y<computationSize.y; y++){
		// 	for(int x=0; x<computationSize.x; x++){
		// 		int idx = x + y*computationSize.x;
		// 		fResult << scaledDepth[0][idx] << "\t";
...
bool Kfusion::tracking(float4 k, float icp_threshold, uint tracking_rate,
	...
	// 			invK);
		...
		// 	for(int i=0; i<3; i++){
		// 		std::string path = "/home/jason/KinectFusion_U50/KinectFusion_HW_impl/output/HW_impl/"; std::string name;
		// 		name = path + "inputNormal" + std::to_string(i) + "_" + std::to_string(counter); 
		// 		std::ofstream fResult(name, std::ios::out);
		// 		for(int y=0; y<240; y++){
		// 			for(int x=0; x<320; x++){

		// 				int k = x + y*320;
		// 				fResult << "(" 	<< inputNormal[i][k].x 
		// 								<< inputNormal[i][k].y 
		// 								<< inputNormal[i][k].z  
		// 								<< inputNormal[i][k].w << ")" << "\t";
		// 			}
	...
	for (int level = iterations.size() - 1; level >= 0; --level) {
		...
		for (int i = 0; i < iterations[level]; ++i) {
			...
			OCL_CHECK(err, err = clEnqueueMigrateMemObjects(Q_trackKernel,(cl_uint)6, pt_tr_in, 0 ,0,NULL, NULL));
			OCL_CHECK(err, err = clEnqueueTask(Q_trackKernel, K_trackANDreduceKernel, 0, NULL, NULL));
			OCL_CHECK(err, err = clEnqueueMigrateMemObjects(Q_trackKernel,(cl_uint)1, pt_tr_out, CL_MIGRATE_MEM_OBJECT_HOST,0,NULL, NULL));

			clFinish(Q_trackKernel);
	...
	for (int j = 1; j < 8; ++j) {
		values[0] += values[j];
			...
			// static int counter=0;
			// std::string path = "/home/jason/KinectFusion_U50/KinectFusion_HW_impl/output/HW_impl/"; std::string name;
			// name = path + "reduce_" + std::to_string(counter); std::ofstream fResult(name, std::ios::out);
			
			// for(int y=0; y<8; y++){
			// 	for(int x=0; x<32; x++){
			// 		int k = x + y*32;
			// 		fResult << reductionoutput[k] << "\t";
			// 	}
			// 		fResult << "\n";
			// }
			// fResult.close();
