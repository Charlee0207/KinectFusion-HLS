...
// unoptimized HW implementation of the combination of halfSampleRobustImageKernel and depth2vertexKernel kernel
...
void halfSampleDepth2vertexKernel(	float4* outVertex0, float4* outVertex1, float4* outVertex2,
	...
	#pragma HLS ARRAY_PARTITION variable=depthBlockLevel0 complete
	float depthBlockLevel1[2][2];
	#pragma HLS ARRAY_PARTITION variable=depthBlockLevel1 complete
	float depthBlockLevel2;

	float4 vertexBlockLevel0[4][4];
	#pragma HLS ARRAY_PARTITION variable=vertexBlockLevel0 complete
	float4 vertexBlockLevel1[2][2];
	#pragma HLS ARRAY_PARTITION variable=vertexBlockLevel1 complete
	...
	#pragma HLS PIPELINE II=1
		...
		#pragma HLS PIPELINE II=1
			...
			#pragma HLS PIPELINE II=1
				for(int j=0; j<4; j++){
				#pragma HLS PIPELINE II=1
					depthBlockLevel0[i][j] = inDepth[(x_offset + y_offset) + (j + inSize_x*i)];
			...
			float centerLv1;
			// Unroll the level of block, and reduce the if logic
			...
			#pragma HLS UNROLL
				for(int j=0; j<2; j++){
					...
					diff_e_dComp0Lv1 = (fabsf(depthBlockLevel0[i*2][j*2] - centerLv1) < e_d);
					diff_e_dComp1Lv1 = (fabsf(depthBlockLevel0[i*2][j*2+1] - centerLv1) < e_d);
					diff_e_dComp2Lv1 = (fabsf(depthBlockLevel0[i*2+1][j*2] - centerLv1) < e_d);
					diff_e_dComp3Lv1 = (fabsf(depthBlockLevel0[i*2+1][j*2+1] - centerLv1) < e_d);

					sum0Lv1 = (diff_e_dComp0Lv1) ? 1.0f : 0.0f; 	t0Lv1 = (diff_e_dComp0Lv1) ? depthBlockLevel0[i*2][j*2] : 0.0f;
					sum1Lv1 = (diff_e_dComp1Lv1) ? 1.0f : 0.0f; 	t1Lv1 = (diff_e_dComp1Lv1) ? depthBlockLevel0[i*2][j*2+1] : 0.0f;
					sum2Lv1 = (diff_e_dComp2Lv1) ? 1.0f : 0.0f; 	t2Lv1 = (diff_e_dComp2Lv1) ? depthBlockLevel0[i*2+1][j*2] : 0.0f;
					sum3Lv1 = (diff_e_dComp3Lv1) ? 1.0f : 0.0f; 	t3Lv1 = (diff_e_dComp3Lv1) ? depthBlockLevel0[i*2+1][j*2+1] : 0.0f;
			...
			float sum0Lv2, sum1Lv2, sum2Lv2, sum3Lv2;
			float t0Lv2, t1Lv2, t2Lv2, t3Lv2;
			bool diff_e_dComp0Lv2, diff_e_dComp1Lv2, diff_e_dComp2Lv2, diff_e_dComp3Lv2;
			// Unroll the level of block, and reduce the if logic
			float centerLv2 = depthBlockLevel1[0][0];

			diff_e_dComp0Lv2 = (fabsf(depthBlockLevel1[0][0] - centerLv2) < e_d);
			diff_e_dComp1Lv2 = (fabsf(depthBlockLevel1[0][1] - centerLv2) < e_d);
			diff_e_dComp2Lv2 = (fabsf(depthBlockLevel1[1][0] - centerLv2) < e_d);
			diff_e_dComp3Lv2 = (fabsf(depthBlockLevel1[1][1] - centerLv2) < e_d);

			sum0Lv2 = (diff_e_dComp0Lv2) ? 1.0f : 0.0f; 	t0Lv2 = (diff_e_dComp0Lv2) ? depthBlockLevel1[0][0] : 0.0f;
			sum1Lv2 = (diff_e_dComp1Lv2) ? 1.0f : 0.0f; 	t1Lv2 = (diff_e_dComp1Lv2) ? depthBlockLevel1[0][1] : 0.0f;
			sum2Lv2 = (diff_e_dComp2Lv2) ? 1.0f : 0.0f; 	t2Lv2 = (diff_e_dComp2Lv2) ? depthBlockLevel1[1][0] : 0.0f;
			sum3Lv2 = (diff_e_dComp3Lv2) ? 1.0f : 0.0f; 	t3Lv2 = (diff_e_dComp3Lv2) ? depthBlockLevel1[1][1] : 0.0f;
			...
			#pragma HLS PIPELINE II=1
				for(int j=0; j<4; j++){
				#pragma HLS PIPELINE II=1
					outVertex0[(x_offset + y_offset) + (j + outSizeLevel0.x*i)] = vertexBlockLevel0[i][j];					
				}
			}
			for(int i=0; i<2; i++){
			#pragma HLS PIPELINE II=1
				for(int j=0; j<2; j++){
				#pragma HLS PIPELINE II=1	
					outVertex1[(x_offset/2 + y_offset/4) + (j + outSizeLevel0.x*i)] = vertexBlockLevel1[i][j];					
				}
			}
