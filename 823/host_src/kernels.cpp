...
cl_context context;
cl_command_queue Q, Q_bilateralFilterKernel, Q_halfSampleDepth2vertexKernel, Q_vertex2normalKernel, Q_trackKernel, Q_integrateKernel;
cl_program program;
cl_kernel K_bilateralFilterKernel, K_halfSampleDepth2vertexKernel, K_vertex2normalKernel, K_trackKernel, K_trackANDreduceKernel, K_integrateKernel;
/////////////////////////////////////////////////////////////////////////
...
void Kfusion::languageSpecificConstructor() {
	...
	// OCL_CHECK(err, K_bilateralFilterKernel = clCreateKernel(program, "mm2meter2bilateralFilterKernel", &err));
	...
	// }
		...
		for (unsigned int i = 0; i < iterations.size(); ++i) {
			...
			for(int i=0; i<3; i++){
				...
				for(int x=0; x<320; x++){
					int k = x + y*320;
					fResult << "(" << inputNormal[i][k].x << inputNormal[i][k].y << inputNormal[i][k].z << inputNormal[i][k].w << ")" << "\t";
				}
					fResult << "\n";
				}
				fResult.close();
